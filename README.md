...
Periodically summarize a long [Claude Code](https://github.com/anthropics/claude-code) chat into project memory, so your other chats do not re-derive the same context.
...
- When a chat crosses the threshold (default 250 tool calls), the hook injects a directive via `hookSpecificOutput.additionalContext` telling Claude to write or update a summary in the project's memory dir, then resets the counter.
...
- Claude Code auto-memory enabled (`autoMemoryEnabled` in settings). This hook writes into the auto-memory directory; without it there is nowhere to write.
- `python3`.
...
The installer copies `hooks/memory_checkpoint.py` into `~/.claude/hooks/` and merges a hook entry into `~/.claude/settings.json` (existing settings and hooks preserved; re-running replaces rather than duplicates). Open `/hooks` once, or restart, to load it.
...
A chat that ends before the threshold will not write a summary (it counts tool calls, not wall-clock time or session end). If you want a guaranteed summary at the end of every chat regardless of length, pair this with a `Stop` hook.
